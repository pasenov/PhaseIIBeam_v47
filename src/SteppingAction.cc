...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
 //get volume of the current step
  ...
  //geometry parameters
  G4double Det1SizeZ, Det2SizeZ, Dist, Strip1Depth, Strip1Length, Strip2Depth, Strip2Length, StripDist, StripWidth, posEndArm1, posBeginningArm2, BPIXSizeZ, pixelDepth, pixelX, pixelY, pixelHalf, ROChor, ROCvert,  XangleDUT, XangleBPIX, YangleBPIX, ElField1, ElField2;
  ...
  G4ThreeVector pointPix1ent, pointPix1ex, pointPix2ent, pointPix2ex, pointDet1ent, pointDet1ex, pointDet2ent, pointDet2ex, pointDet3rent, pointDet4rent;
  ...
  if (track->GetTrackID() == 1)  {
	...
	if (pre->GetStepStatus() == fGeomBoundary)   {
	    ...
	    momDir1z = track->GetMomentumDirection().z();
    	    ...
    	    fEventaction->AddAReal(pointPix1ent);
    ...
    /*if (volume == fDetectorconstruction->GetDet3r())   {
	if (pre->GetStepStatus() == fGeomBoundary)   {
	    pointDet3rent = pre->GetPosition();

    	    fEventaction->AddBr1Real(pointDet3rent);

            //G4cout 
               //<< "\n Primary has entered reduced module 3 at: " 
               //<< G4BestUnit(pointDet3rent, "Length")
               //<< G4endl;
        }
	...
	if (pre->GetStepStatus() == fGeomBoundary)   {
	    pointDet1ent = pre->GetPosition();
    	    ...
    	    fEventaction->AddBReal(pointDet1ent);
	...
	if (pre->GetStepStatus() == fGeomBoundary)   {
	    pointDet2ent = pre->GetPosition();
    	    ...
    	    fEventaction->AddCReal(pointDet2ent);
    ...
    /*if (volume == fDetectorconstruction->GetDet4r())   {
	if (pre->GetStepStatus() == fGeomBoundary)   {
	    pointDet4rent = pre->GetPosition();

    	    fEventaction->AddCr1Real(pointDet4rent);

            //G4cout 
               //<< "\n Primary has entered reduced module 4 at: " 
               //<< G4BestUnit(pointDet4rent, "Length")
               //<< G4endl;
        }
	...
	if (pre->GetStepStatus() == fGeomBoundary)   {
	    pointPix2ent = pre->GetPosition();
    	    ...
    	    fEventaction->AddDReal(pointPix2ent);
 ...
 if (volume == fDetectorconstruction->GetBPIX12()) {
   if ((zPix >= (posEndArm1 - BPIXSizeZ))  &&  (zPix <= (posEndArm1 - BPIXSizeZ + pixelDepth))) {
	...
	col = 433 - div1;
     ...
     if ((row>=1) && (row<217) && (col>=1) && (col<865))   {
	   fEventaction->AddEnergyPixel1(edep,row,col);
     }
   }
 }
